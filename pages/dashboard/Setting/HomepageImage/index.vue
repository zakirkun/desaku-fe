...
export default {
    ...
    methods: {
        async loadData() {
            const data = await $fetch(this.$config.public.PUBLIC_API_BASE_URL + '/api/image-homepage')
            this.items = data
        },
        ...
        async removeNews() {
            await $fetch(this.$config.public.PUBLIC_API_BASE_URL + '/api/image-homepage/' + this.removedImageId, {
                method: "DELETE",
            })
